...
primary <- chair_sum %>%
  mutate(nDes = log(nDes+1),
         ...
         majority = as.factor(majority)) %>% ungroup() %>% select(-blank, -invalid)
...
des2 <- primary %>% ggplot(aes(nDes)) + 
  ...
  labs(x = "log Chairman decisions",
       y = "")
des3 <- chair_sum %>% ggplot(aes(sqrt(nDes)))+ 
  geom_histogram()+
  labs(x = "square root Chairman decisions",
       y = "")
des4 <- chair_sum %>% ggplot(aes(nDes^(1/3)))+ 
  geom_histogram()+
  labs(x = "cube root Chairman decisions",
       y = "")
#in paper
plot_grid(des1, des2, des3, des4,
          labels = c("A", "B", "C", "D"))
```
...
m2_pooled <- plm(
  nDes ~ majority + pop + median_income + province,
  data = primary,
  model = "pooling",
  index = c("muni", "year"))
...
m3_individual <- plm(
  nDes ~ majority + pop + median_income + province + nRule + C_part_of_rule + L_part_of_rule + KD_part_of_rule + MP_part_of_rule + 
...
m1_se <- list(sqrt(diag(vcovHC(m1_individual, type = "HC3", method = "arellano"))),
              sqrt(diag(vcovHC(m1_time, type = "HC3", method = "arellano"))),
              sqrt(diag(vcovHC(m2_individual, type = "HC3",  method = "arellano"))),
              sqrt(diag(vcovHC(m2_time, type = "HC3",  method = "arellano"))),
              sqrt(diag(vcovHC(m3_individual, type = "HC3",  method = "arellano"))),
              sqrt(diag(vcovHC(m3_time, type = "HC3",  method = "arellano")))
              )

stargazer(m1_individual, m1_time, m2_individual, m2_time, m3_individual, m3_time,
          se = m1_se,
          notes = "Robust Standard Error in Parenthesis",
          type = "html",
          out = "./Regressions/robust.html")

stargazer(m1_individual, m1_time, m2_individual, m2_time, m3_individual, m3_time, 
          notes = "Standard Error in Parenthesis",
          type = "html",
          out = "./Regressions/standard.html")
```

### Testing the models
```{r}
#F test for time effects
pFtest(m2_time, m2_pooled)

#Lagrange Multiplier Test - time effects (Breusch-Pagan) for balanced panels
plmtest(m2_individual, "time", type = "bp")

#Breusch-Pagan LM test for cross-sectional dependence in panels
pcdtest(m2_individual, test = "lm")

#Pesaran CD test for cross-sectional dependence in panels
pcdtest(m2_individual, test = "cd")

#Breusch-Godfrey/Wooldridge test for serial correlation in panel models
pbgtest(m2_individual)
pbgtest(m2_time)
